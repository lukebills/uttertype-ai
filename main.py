...
def format_with_context(text: str) -> str:
    ...
    try:
        ...
        logger.error(f"Error formatting text with context: {e}", exc_info=True)
        return text.strip()
...
def format_for_email(text: str) -> str:
    ...
    try:
        ...
        logger.error(f"Error formatting email text: {e}", exc_info=True)
        return text.strip()

def handle_email_formatting():
    """Handle the Ctrl+Alt+Shift+E shortcut for email formatting."""
    try:
        logger.info("Starting email formatting process")
        ...
        logger.debug(f"Original clipboard length: {len(original_clipboard)}")
        ...
        if selected_text.strip() and selected_text != original_clipboard:
            ...
            logger.info(f"Processing existing clipboard text: {selected_text[:50]}...")
            formatted_text = format_for_email(selected_text)
            pyperclip.copy(formatted_text)
            pyautogui.hotkey(copy_key, "v")
            logger.info("Email formatting completed")
        else:
            logger.warning("No text available for email formatting")
            # Restore original clipboard if nothing was selected
            pyperclip.copy(original_clipboard)
            
    except Exception as e:
        logger.error(f"Error in email formatting: {e}", exc_info=True)
...
def main():
    ...
    logger.info("UtterType AI started successfully")
    logger.info("Voice transcription: Ctrl+Alt+Q (basic) | Ctrl+Alt+A (AI formatting)")
    logger.info("Email formatting: Ctrl+Alt+Shift+E (select text first)")
    
    try:
        loop.run_forever()
    except KeyboardInterrupt:
        hotkey.stop_listeners()
        logger.info("UtterType AI shutting down...")
